...
import { whenToUseSection } from "@/lib/agent-guidance"

/**
 * Contenu de /llms.txt et /llms-full.txt, généré depuis les données du site :
 * plus aucun lien mort possible, et les nouvelles pages (blog, /ia/*, glossaire…)
 * apparaissent automatiquement.
 *
 * Deux variantes pour une raison précise : un llms.txt est un index de navigation
 * et la convention le veut sous 30 000 caractères. Avec les descriptions de chaque
 * article, terme de glossaire et métier, le fichier dépassait 37 000 — assez pour
 * qu'un agent le tronque et perde la fin. L'index garde donc **tous les liens**
 * (aucune page ne devient inatteignable) mais réserve les descriptions longues à
 * /llms-full.txt.
 */
export type LlmsVariant = "index" | "full"
...
export function buildLlmsContent(variant: LlmsVariant): string {
  const lines: string[] = []
  const isFull = variant === "full"
  /** Suffixe descriptif d'un lien : omis dans l'index, qui ne garde que la navigation. */
  const detail = (text: string) => (isFull ? `: ${text}` : "")
  ...
  lines.push(whenToUseSection())
  lines.push("")
  if (!isFull) {
    lines.push(
      "> Index de navigation. Les descriptions détaillées de chaque page se trouvent dans [/llms-full.txt](/llms-full.txt)."
    )
    lines.push("")
  }
  ...
  for (const service of services) {
    lines.push(
      `- [${service.jsonLd.name}](/services/${service.slug})${detail(service.jsonLd.description)}`
    )
  }
  lines.push("")
  ...
  for (const category of aiCategoryOrder) {
    ...
    for (const profile of profiles) {
      lines.push(
        `- [IA pour les ${profile.namePlural}](/ia/${profile.slug})${detail(
          `${profile.answerFirst.what.answer.split(".")[0]}.`
        )}`
      )
    }
    lines.push("")
  ...
  for (const [category, categoryPosts] of postsByCategory) {
    ...
    for (const post of categoryPosts) {
      lines.push(`- [${post.title}](/blog/${post.slug})${detail(post.description)}`)
    }
    lines.push("")
  }
  ...
  for (const expertise of getAllExpertises()) {
    lines.push(
      `- [${expertise.name}](/expertise/${expertise.slug})${detail(expertise.metaDescription)}`
    )
  }
  lines.push("")
  ...
  for (const [category, categoryTerms] of termsByCategory) {
    ...
    for (const term of categoryTerms) {
      lines.push(`- [${term.term}](/glossaire/${term.slug})${detail(term.definition)}`)
    }
    lines.push("")
  }
